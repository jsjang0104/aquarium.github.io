...
친구들의 얼굴을 입힌 물고기 일곱 마리가 헤엄치는 인터랙티브 3D 아쿠아리움입니다. HTML, CSS, JavaScript와 Three.js를 사용하며 빌드 과정이나 백엔드 없이 정적 호스팅으로 실행합니다.
...
- **주민 일곱 명:** 기본 JPEG 사진 7장이 물고기 머리의 앞면에 직접 매핑됩니다. 얼굴이 둥근 곡면을 따라 휘고 조명을 받으며, 사진 가장자리는 몸 색과 부드럽게 섞입니다.
...
- **한판 붙자:** 민트·코랄 팀에 서로 다른 두 마리를 고르고 ‘대결 시작’을 누르세요. 중앙에서 회전하고 돌진한 뒤 무작위 승자의 세리머니가 나옵니다. 기본 속도에서 약 13초이며 헤엄 속도에 따라 달라집니다. 일시정지는 대결도 멈추고, ‘그만 싸워’ 또는 먹이 주기는 대결을 끝냅니다. 결과는 저장하지 않습니다.
...
모든 방문자는 `js/portraits.js`의 `DEFAULT_FISH`에 정의된 같은 일곱 마리를 봅니다. 페이지에서 사진이나 물고기를 추가·수정·삭제하는 기능은 없습니다. 주민 카드는 보기 전용입니다.
...
js/fish-duel.js       선택한 두 물고기의 대결·승패·효과 정리
...
기본 렌더링과 제어, 이전 저장값 무시, 저장소 접근 없이 실행, 보기 전용 주민, 모바일과 움직임 감소 설정, 키보드 먹이 주기, 장시간 유영 경계와 먹이 섭취·정리, 앞면 사진의 곡면 일치와 방향, 놀이 반응·기포 정리, 마우스·멀티터치·터치 취소, 대결 선택·취소·종료·먹이 전환, WebGL 오류 안내를 검사합니다. `AQUARIUM_URL` 환경 변수로 배포 주소에서도 실행할 수 있습니다.
